...
- Detect tumbler product specs and flat-bed items from photos using Codex Vision
...
| AI / Vision | @anthropic-ai/sdk 0.80 (Codex Vision API) |
...
src/
  app/
    ...
    api/admin/
      tumbler/auto-size/         → POST: Codex Vision tumbler analysis
      flatbed/auto-detect/       → POST: Codex Vision flat-bed item detection
  ...
  server/
    tumbler/
      ...
      claudeVisionAnalysis.ts    → Codex Vision API call for tumbler
    ...
    flatbed/
      analyzeFlatBedWithVision.ts → Codex Vision API call for flat-bed items
...
| `TumblerAutoDetectPanel` | Upload tumbler photo → Codex Vision → parse specs → apply to bed config. **Only shown in tumbler-wrap mode.** |
| `FlatBedAutoDetectPanel` | Upload product photo → Codex Vision → match flat-bed catalog item → apply overlay. **Only shown in flat-bed mode.** |
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/api/admin/tumbler/auto-size` | Accepts `multipart/form-data` with `image` field; calls Codex Vision; returns `TumblerAutoSizeResponse` |
